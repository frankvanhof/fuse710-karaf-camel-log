...
. Import base images in your newly created project (karaf-log) according to https://access.redhat.com/documentation/en-us/red_hat_fuse/7.10/html/fuse_on_openshift_guide/get-started-non-admin[documentation].
...
[source,bash,options="nowrap",subs="attributes+"]
----
$ mvn clean -DskipTests oc:deploy -Popenshift -Djkube.generator.from=karaf-log/fuse-karaf-openshift:1.10
----

. In your browser, navigate to the `karaf-log` project in the OpenShift console.
Wait until you can see that the pod for the `karaf-camel-log` has started up.

. On the project's `Overview` page, navigate to the details page deployment of the `karaf-camel-log` application: `https://OPENSHIFT_IP_ADDR:8443/console/project/karaf-log/browse/pods/karaf-camel-log-NUMBER_OF_DEPLOYMENT?tab=details`.
